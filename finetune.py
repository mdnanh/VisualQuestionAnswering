...
from utils import AverageMeter, get_autocast, get_cast_dtype, get_checkpoint
...
def main():
    ...
    parser.add_argument("--lm_path", default="vilm/vinallama-2.7b", type=str)
    parser.add_argument(
        "--tokenizer_path",
        default="vilm/vinallama-2.7b",
        type=str,
    ...
    if args.rank == 0:
        torch.save(
            {"model_state_dict": get_checkpoint(model), "tuning_config": tuning_config},
...
def train_one_epoch(
    ...
    for num_steps, batch in tqdm(
        ...
        #### VISION FORWARD PASS ####
        images = batch["image"].to(device, dtype=cast_dtype, non_blocking=True).unsqueeze(1).unsqueeze(1)
        input_ids = batch["input_ids"].to(device, dtype=cast_dtype, non_blocking=True)
        attention_mask = batch["attention_mask"].to(device, dtype=cast_dtype, non_blocking=True)
        labels = batch["labels"].to(device, dtype=cast_dtype, non_blocking=True)
